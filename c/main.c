...
void wifi_csi_rx_cb(void *ctx, wifi_csi_info_t *data) {
    ...
    for (size_t i = 0; i < sample_count; i++) {
        ...
        printf("[%ld]: real: %d, imag: %d, magnitude: %d\n", i, real, imag, (uint16_t)magnitude);
...
int main() {
    ...
    csi_info.len = sizeof(buffer);
    csi_info.buf = (int8_t *)malloc(csi_info.len * sizeof(int8_t));
    ...
    printf("[csi_info.len]: %d\n", csi_info.len);

    memcpy(csi_info.buf, buffer, csi_info.len);
